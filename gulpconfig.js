...
module.exports = {
    ...
    scripts: {
        ...
        chunks: { // Chunks are arrays of globs matching source files that combine to provide specific functionality
            ...
            gallery: [src + 'js/gallery_init.js'],
            shuffle: [src + 'js/jquery.shuffle.modernizr.min.js'],
            admin: [src + 'js/admin.ajax.js']
        },
    ...
    styles: {
        ...
        libsass: { // Requires the libsass implementation of Sass
            includePaths: [bower, bower + 'bootstrap-sass/assets/stylesheets/'], // Adds the `bower_components` directory to the load path so you can @import directly
            precision: 6,
    ...
    vendor: {
        bowerIncludes: [
            bower + 'Custom-Meta-Boxes/**/*',
            bower + 'essence/**/*',
            bower + 'wp-bootstrap-navwalker/**/*',
            bower + 'blueimp-gallery/**/*',
            bower + 'jquery.tocify.js/**/*',
            bower + 'redux-framework/**/*'
        ],
        srcIncludes: [
          src + 'fonts/**/*'
        ],
