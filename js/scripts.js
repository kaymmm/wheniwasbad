...
jQuery(document).ready(function($) {
    ...
    // Title waypoints to stick heading to the header
    // add the class ".waypoint-title" to enable
    $('.waypoint-title').each(function() {
        if ( $(document).width() >= 768 && $('#main_header').hasClass('navbar-fixed-top')) {
            var $this = $(this);
            $this.waypoint(function(direction) {
                $('#main_header .subhead.navbar-brand').remove();
                var title;
                if (direction === 'down') {
                    title = $this.html();
                } else {
                    ...
                    if (prev.length > 0) {
                        title = $this.waypoint('prev').html(); 
                    } else {
                        return 0;
                    }
                }
                var elm = '<span class="subhead navbar-brand"><i class="fa fa-angle-right" style="padding-right: 15px;"></i>';
                elm += title;
                elm += '</span>';
                $('#logo').after(elm);
                $('#main_header .subhead.navbar-brand').fadeIn("fast");
            },
            { offset : $('#main_header').position().top + $('#main_header').height() });
        }
    });
